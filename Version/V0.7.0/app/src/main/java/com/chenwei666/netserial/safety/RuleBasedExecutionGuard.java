...
public final class RuleBasedExecutionGuard implements ExecutionGuard {
    ...
    private RiskLevel classify(String command) {
        RiskLevel highest = RiskLevel.R0_INFORMATIONAL;
        String[] lines = command.replace('\r', '\n').split("\\n");
        for (String line : lines) highest = RiskLevel.max(highest, classifyLine(line));
        return highest;
    }

    private RiskLevel classifyLine(String command) {
        String normalized = command.trim().toLowerCase(Locale.ROOT);
        if (matchesAny(normalized, "reboot", "reload", "reset saved-configuration",
                "erase startup-config", "write erase", "format", "upgrade", "bootrom", "delete /unreserved")) {
