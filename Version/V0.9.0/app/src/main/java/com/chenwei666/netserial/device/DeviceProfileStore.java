...
public final class DeviceProfileStore {
    ...
    public DeviceProfile load() {
        try {
            return new DeviceProfile(
                    ...
                    DeviceEnvironment.valueOf(preferences.getString("environment", DeviceEnvironment.LAB.name())),
                    preferences.getBoolean("protected_device", false),
                    preferences.getString("management_address", "")
            );
    ...
    public void save(DeviceProfile profile) {
        ...
        boolean saved = preferences.edit()
                ...
                .putString("environment", profile.getEnvironment().name())
                .putBoolean("protected_device", profile.isProtectedDevice())
                .putString("management_address", profile.getManagementAddress())
                .commit();
        if (!saved) {
