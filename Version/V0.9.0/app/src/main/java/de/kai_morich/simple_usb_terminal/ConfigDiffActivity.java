...
import com.chenwei666.netserial.config.ConfigDriftAssessment;
import com.chenwei666.netserial.config.ConfigNormalizer;
import com.chenwei666.netserial.config.ConfigSnapshot;
import com.chenwei666.netserial.config.ConfigurationDriftEngine;
...
public final class ConfigDiffActivity extends ThemedActivity {
    ...
    public static final String EXTRA_BEFORE = "before";
    ...
    @Override protected void onCreate(Bundle state) {
        ...
        String explicitBefore = sanitize(getIntent().getStringExtra(EXTRA_BEFORE));
        String explicitAfter = sanitize(getIntent().getStringExtra(EXTRA_AFTER));
        if (!explicitBefore.isEmpty()) before.setText(explicitBefore);
        else if (!capture.isEmpty()) before.setText(capture);
        if (!explicitAfter.isEmpty()) after.setText(explicitAfter);
    ...
    private void compare() {
        try {
            ...
            ConfigDriftAssessment drift = new ConfigurationDriftEngine().assess(left, right);
            ...
            latestReport = getString(R.string.config_report_title)
                    ...
                    + "\n" + getString(R.string.config_drift_summary, drift.getSeverity().name(),
                    drift.getTotalChanges(), drift.getSensitiveChanges())
