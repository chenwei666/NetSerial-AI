...
V0.9.0 提供 18 个厂商/接入类型入口。预设用于减少配置工作，不会锁死端点或模型；用户仍可覆盖 HTTPS Endpoint、模型名和安全凭据别名。所有入口同时用于命令规划与应用内多轮对话。
...
- 厂商兼容不代表能力完全一致；当前使用文本命令规划与多轮文本对话的共同字段。
- 图片、文件、工具调用、厂商联网搜索及厂商侧会话持久化不属于 V0.9.0 范围。
...
V0.9.0 exposes 18 provider or gateway entries for both guarded command planning and in-app multi-turn chat. Sixteen cloud presets and custom gateways use the OpenAI-compatible text interface, Anthropic uses its native Messages authentication, and Ollama uses the OpenAI-compatible chat interface plus `/api/tags` for local model discovery. Upstream discovery and chat are bounded, cancellable, HTTPS-only, redirect-free, and separated from encrypted credentials.
