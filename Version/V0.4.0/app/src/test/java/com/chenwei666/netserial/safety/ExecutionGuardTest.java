...
public class ExecutionGuardTest {
    ...
    @Test public void multilineBatchCannotHideCriticalCommandBehindReadOnlyLine() {
        GuardDecision decision = RuleBasedExecutionGuard.createDefault().evaluate(
                new CommandEvaluationRequest(Vendor.H3C_COMWARE, CliMode.USER_VIEW,
                        "display version\nreboot", RiskLevel.R0_INFORMATIONAL));
        assertEquals(RiskLevel.R4_CRITICAL, decision.getEffectiveRisk());
    }

    @Test public void commandBatchCountIgnoresBlankLines() {
        assertEquals(2, new CommandBatchInspector().count("display version\n\nreboot\n"));
    }
}
