# NetSerial AI Operations Terminal V0.3.1

[中文](README.md) | [English](README_EN.md)

V0.3.1 is the first production-signed release. It retains USB serial, multi-provider AI, device memory, and offline completion while including separate SSH/Telnet transport, a categorized command library, complete app settings, and a reproducible secure release flow.
...
# On the signing workstation after production-signing initialization:
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\build.ps1 -Release
```

The script mirrors the project to `C:\tmp\NetSerial-build`. A normal build runs `testDebugUnitTest`, `lintDebug`, and `assembleDebug`; a production build runs `testDebugUnitTest`, `lintRelease`, and `assembleRelease`. JDK 17, Android SDK 36, and the current Windows user's production signing identity are required. See `docs/RELEASE.md` for initialization and backup guidance.
...
- V0.3.1 has a verified production-signed Release APK; physical-device upgrade and real-switch matrix acceptance remain external checks.
