...
import com.chenwei666.netserial.terminal.ActiveTerminalSessionStore;
import com.chenwei666.netserial.terminal.TerminalControlEncoder;
import com.chenwei666.netserial.terminal.TerminalSessionTransport;
...
import com.chenwei666.netserial.transfer.XmodemSender;
...
public class TerminalFragment extends Fragment implements ServiceConnection, SerialListener {
    ...
    private static final int OPERATIONS_REQUEST = 4104;
    private static final int XMODEM_FILE_REQUEST = 4105;
    ...
    private final String terminalSessionId = "usb-" + UUID.randomUUID();
    ...
    private final DeviceFingerprintEngine fingerprintEngine = new DeviceFingerprintEngine();
    private volatile XmodemSender activeXmodem;
    private boolean vendorDetectionCompleted;
    ...
    public void onDestroy() {
        if (activeXmodem != null) activeXmodem.cancel();
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.clear) {
            ...
            ActiveTerminalSessionStore.shared().clear(
                    terminalSessionId, System.currentTimeMillis());
            return true;
            ...
            return true;
        } else if (id == R.id.ai_diagnose) {
            Intent diagnose = new Intent(requireContext(), AiCopilotActivity.class)
                    .putExtra(AiCopilotActivity.EXTRA_TERMINAL_CONTEXT, terminalBuffer.tail(12_000))
                    .putExtra(AiCopilotActivity.EXTRA_DIAGNOSTIC_TEXT, terminalBuffer.tail(8_000));
            startActivityForResult(diagnose, AI_COPILOT_REQUEST);
            return true;
            ...
            return true;
        } else if (id == R.id.web_access) {
            WebAccessWizardDialog.show(requireActivity(), deviceProfile, detectedPlatform,
                    this::sendSensitiveWebPlan);
            return true;
        } else if (id == R.id.xmodem_send) {
            chooseXmodemFile();
            return true;
        }
    ...
    private void disconnect() {
        ...
        ActiveTerminalSessionStore.shared().disconnected(
                terminalSessionId, System.currentTimeMillis());
    ...
    private void sendUnchecked(String str) {
        ...
        try {
            ...
            ActiveTerminalSessionStore.shared().append(
                    terminalSessionId, msg + "\n", System.currentTimeMillis());
    ...
    private void sendSensitiveWebPlan(WebAccessPlan plan) {
        if (connected != Connected.True) {
            plan.destroy();
            Toast.makeText(getActivity(), R.string.remote_disconnected, Toast.LENGTH_SHORT).show();
            return;
        ...
        DeviceProfile currentProfile = new DeviceProfileStore(requireContext()).load();
        TargetSafetyDecision targetDecision = new TargetSafetyPolicy().evaluate(currentProfile,
                currentProfile.getName(), RiskLevel.R3_HIGH,
                new ChangeTaskStore(requireContext()).loadActive(), System.currentTimeMillis());
        if (!targetDecision.isAllowed()) {
            plan.destroy();
            new AlertDialog.Builder(requireContext()).setTitle(R.string.target_blocked_title)
                    .setMessage(getString(R.string.target_blocked_message, targetLabel(),
                            TargetSafetyMessageResolver.resolve(requireContext(), targetDecision.getReason())))
                    .setPositiveButton(android.R.string.ok, null).show();
            return;
        }
        CommandSendConfirmation.confirm(requireContext(),
                getString(R.string.target_command_summary, targetLabel(), plan.getCommands().size())
                        + "\n\n" + plan.redactedBatch(), RiskLevel.R3_HIGH,
                () -> {
                    try { sendSensitiveUnchecked(plan.commandBatch(), plan.redactedBatch()); }
                    finally { plan.destroy(); }
                }, plan::destroy);
    }

    private void sendSensitiveUnchecked(String wireCommand, String redactedCommand) {
        flushEvidenceOutput();
        byte[] data = (wireCommand + newline).getBytes();
        try {
            SpannableStringBuilder spn = new SpannableStringBuilder(redactedCommand + '\n');
            ...
            receiveText.append(spn);
            terminalBuffer.append(redactedCommand + "\n");
            ActiveTerminalSessionStore.shared().append(
                    terminalSessionId, redactedCommand + "\n", System.currentTimeMillis());
            new ChangeEvidenceRecorder(requireContext()).record(ChangeEventType.COMMAND_SENT,
                    targetLabel(), redactedCommand);
            trimTerminalView();
            service.write(data);
            java.util.Arrays.fill(data, (byte) 0);
        } catch (Exception exception) {
            java.util.Arrays.fill(data, (byte) 0);
            onSerialIoError(exception);
        }
    }
    ...
    private void receive(ArrayDeque<byte[]> datas) {
        ...
        ActiveTerminalSessionStore.shared().append(
                terminalSessionId, spn, System.currentTimeMillis());
        appendEvidenceOutput(spn.toString());
        detectVendorIfNeeded();
        trimTerminalView();
    }

    private void detectVendorIfNeeded() {
        if (vendorDetectionCompleted || getContext() == null) return;
        DeviceFingerprint fingerprint = fingerprintEngine.identify(terminalBuffer.tail(16_000));
        if (!fingerprint.isHighConfidence()) return;
        vendorDetectionCompleted = true;
        detectedPlatform = fingerprint.getPlatform();
        if (fingerprint.getVendor() != deviceProfile.getVendor()) {
            deviceProfile = deviceProfile.withVendor(fingerprint.getVendor());
            new DeviceProfileStore(requireContext()).save(deviceProfile);
            renderTargetBanner();
        }
        Toast.makeText(requireContext(), getString(R.string.vendor_detected,
                deviceProfile.getVendor().name(), fingerprint.getConfidence()), Toast.LENGTH_LONG).show();
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        } else if (requestCode == OPERATIONS_REQUEST && resultCode == Activity.RESULT_OK && data != null) {
            String command = data.getStringExtra(OperationsCenterActivity.RESULT_COMMAND_BATCH);
            if (command != null) {
                sendText.setText(command);
                sendText.setSelection(command.length());
            }
        } else if (requestCode == XMODEM_FILE_REQUEST && resultCode == Activity.RESULT_OK
                && data != null && data.getData() != null) {
            prepareXmodem(data.getData());
    ...
    private void chooseXmodemFile() {
        if (connected != Connected.True || activeXmodem != null) {
            Toast.makeText(requireContext(), R.string.xmodem_not_ready, Toast.LENGTH_LONG).show();
            return;
        }
        DeviceProfile current = new DeviceProfileStore(requireContext()).load();
        TargetSafetyDecision decision = new TargetSafetyPolicy().evaluate(current, current.getName(),
                RiskLevel.R3_HIGH, new ChangeTaskStore(requireContext()).loadActive(),
                System.currentTimeMillis());
        if (!decision.isAllowed()) {
            new AlertDialog.Builder(requireContext()).setTitle(R.string.target_blocked_title)
                    .setMessage(getString(R.string.target_blocked_message, targetLabel(),
                            TargetSafetyMessageResolver.resolve(requireContext(), decision.getReason())))
                    .setPositiveButton(android.R.string.ok, null).show();
            return;
        }
        startActivityForResult(new Intent(Intent.ACTION_OPEN_DOCUMENT).setType("*/*")
                .addCategory(Intent.CATEGORY_OPENABLE), XMODEM_FILE_REQUEST);
    }

    private void prepareXmodem(Uri uri) {
        status(getString(R.string.xmodem_reading));
        new Thread(() -> {
            try {
                byte[] bytes = readBounded(uri, 16 * 1024 * 1024);
                String hash = sha256(bytes);
                mainLooper.post(() -> confirmXmodem(bytes, hash));
            } catch (Exception exception) {
                mainLooper.post(() -> status(getString(R.string.xmodem_failed)));
            }
        }, "xmodem-file-read").start();
    }

    private void confirmXmodem(byte[] bytes, String hash) {
        if (!isAdded()) return;
        new AlertDialog.Builder(requireContext()).setTitle(R.string.xmodem_confirm_title)
                .setMessage(getString(R.string.xmodem_confirm_message, bytes.length, hash))
                .setNegativeButton(android.R.string.cancel, (dialog, which) ->
                        java.util.Arrays.fill(bytes, (byte) 0))
                .setOnCancelListener(dialog -> java.util.Arrays.fill(bytes, (byte) 0))
                .setPositiveButton(R.string.xmodem_start, (dialog, which) -> startXmodem(bytes))
                .show();
    }

    private void startXmodem(byte[] bytes) {
        DeviceProfile current = new DeviceProfileStore(requireContext()).load();
        TargetSafetyDecision decision = new TargetSafetyPolicy().evaluate(current, current.getName(),
                RiskLevel.R3_HIGH, new ChangeTaskStore(requireContext()).loadActive(),
                System.currentTimeMillis());
        if (!decision.isAllowed() || connected != Connected.True) {
            java.util.Arrays.fill(bytes, (byte) 0);
            Toast.makeText(requireContext(), R.string.xmodem_not_ready, Toast.LENGTH_LONG).show();
            return;
        }
        ...
        activeXmodem = sender;
        status(getString(R.string.xmodem_wait_receiver));
        new Thread(() -> {
            try {
                sender.send(bytes, value -> service.write(value), (sent, total) ->
                        mainLooper.post(() -> status(getString(R.string.xmodem_progress, sent, total))));
                mainLooper.post(() -> status(getString(R.string.xmodem_complete)));
            } catch (Exception exception) {
                mainLooper.post(() -> status(getString(R.string.xmodem_failed)));
            } finally {
                java.util.Arrays.fill(bytes, (byte) 0);
    ...
    private byte[] readBounded(Uri uri, int maximum) throws IOException {
        try (InputStream input = requireContext().getContentResolver().openInputStream(uri);
             ByteArrayOutputStream output = new ByteArrayOutputStream()) {
            if (input == null) throw new IOException("open failed");
            byte[] buffer = new byte[16_384];
            int read;
            while ((read = input.read(buffer)) != -1) {
                if (output.size() + read > maximum) throw new IOException("file too large");
                output.write(buffer, 0, read);
            }
            if (output.size() == 0) throw new IOException("empty file");
    ...
    public void onSerialConnect() {
        ...
        ActiveTerminalSessionStore.shared().connected(terminalSessionId,
                TerminalSessionTransport.USB, targetLabel(), "",
                System.currentTimeMillis());
        status("connected");
    ...
    public void onSerialRead(byte[] data) {
        XmodemSender sender = activeXmodem;
        if (sender != null) {
            sender.accept(data);
            return;
        }
    ...
    public void onSerialRead(ArrayDeque<byte[]> datas) {
        XmodemSender sender = activeXmodem;
        if (sender != null) {
            for (byte[] data : datas) sender.accept(data);
            return;
        }
        receive(datas);
    }
