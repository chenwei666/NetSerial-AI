...
public class RemoteTerminalActivity extends ThemedActivity implements RemoteConnectionListener {
    public static final String EXTRA_PROTOCOL = "remote_protocol";
    public static final String EXTRA_HOST = "remote_host";
    public static final String EXTRA_PORT = "remote_port";
    public static final String EXTRA_USERNAME = "remote_username";
    public static final String EXTRA_SESSION_LABEL = "remote_session_label";
    ...
    private static final int OPERATIONS_REQUEST = 5206;
    ...
    private EditText credentialAliasInput;
    ...
    private final String terminalSessionId = "remote-" + UUID.randomUUID();
    ...
    private void applySessionProfile() {
        ...
        if (label != null && !label.trim().isEmpty()) setTitle(label.trim());
        String protocol = intent.getStringExtra(EXTRA_PROTOCOL);
        protocolSpinner.setSelection(RemoteProtocol.TELNET.name().equals(protocol) ? 1 : 0);
        String host = intent.getStringExtra(EXTRA_HOST);
        String username = intent.getStringExtra(EXTRA_USERNAME);
        int port = intent.getIntExtra(EXTRA_PORT, -1);
        if (host != null && !host.trim().isEmpty()) hostInput.setText(host.trim());
        if (username != null) usernameInput.setText(username.trim());
        if (port > 0 && port <= 65535) portInput.setText(String.valueOf(port));
    }
    ...
    private void bindViews() {
        ...
        credentialAliasInput = findViewById(R.id.remote_credential_alias);
    ...
    private void bindActions() {
        ...
        findViewById(R.id.remote_ai_diagnose).setOnClickListener(v -> {
        ...
        findViewById(R.id.remote_web_access).setOnClickListener(v ->
                WebAccessWizardDialog.show(this, new DeviceProfileStore(this).load(), detectedPlatform,
                        this::sendSensitiveWebPlan));
        findViewById(R.id.remote_operations).setOnClickListener(v ->
                startActivityForResult(new Intent(this, OperationsCenterActivity.class)
        ...
        findViewById(R.id.remote_credential_vault).setOnClickListener(v ->
                startActivity(new Intent(this, AdvancedToolkitActivity.class)));
    ...
    private void protocolChanged(int position) {
        ...
        credentialAliasInput.setEnabled(ssh);
    ...
    private void connect(RemoteProtocol protocol) {
        try {
            ...
            String alias = credentialAliasInput.getText().toString().trim();
            if (protocol == RemoteProtocol.SSH && !alias.isEmpty()) {
                if (android.os.Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.M) {
                    connection = null;
                    Toast.makeText(this, R.string.advanced_keystore_unavailable, Toast.LENGTH_LONG).show();
                    return;
                }
                try {
                    CredentialVaultFactory.create(this).withCredential(
                            DeviceCredentialAliases.vaultKey(alias), password -> {
                                startConnection(password);
                                return null;
                            });
                } catch (RuntimeException exception) {
                    abandonConnection();
                    Toast.makeText(this, R.string.remote_credential_unavailable, Toast.LENGTH_LONG).show();
                    return;
                }
            } else {
    ...
    private void startConnection(char[] password) {
    ...
    private void sendSensitiveWebPlan(WebAccessPlan plan) {
        ...
        CommandSendConfirmation.confirm(this,
                getString(R.string.target_command_summary, targetLabel(profile), plan.getCommands().size())
                        + "\n\n" + plan.redactedBatch(), RiskLevel.R3_HIGH,
                () -> sendSensitiveNow(plan), plan::destroy);
    }

    private void sendSensitiveNow(WebAccessPlan plan) {
        byte[] data = new byte[0];
        try {
            data = (plan.commandBatch() + "\r\n").getBytes(settings.getRemoteCharset().equals("GBK")
                    ? java.nio.charset.Charset.forName("GBK") : java.nio.charset.Charset.forName(settings.getRemoteCharset()));
            sendBytes(data);
            outputText.append("\n" + plan.redactedBatch() + "\n");
            terminalBuffer.append(plan.redactedBatch() + "\n");
            ActiveTerminalSessionStore.shared().append(terminalSessionId,
                    plan.redactedBatch() + "\n", System.currentTimeMillis());
            DeviceProfile profile = new DeviceProfileStore(this).load();
            new ChangeEvidenceRecorder(this).record(ChangeEventType.COMMAND_SENT,
                    targetLabel(profile), plan.redactedBatch());
        } finally {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode != COMMAND_REQUEST && requestCode != AI_REQUEST
                && requestCode != OPERATIONS_REQUEST) return;
        String command = requestCode == COMMAND_REQUEST
                ? data.getStringExtra(CommandLibraryActivity.RESULT_COMMAND)
                : requestCode == AI_REQUEST
                ? data.getStringExtra(AiCopilotActivity.EXTRA_SELECTED_COMMAND)
                : data.getStringExtra(OperationsCenterActivity.RESULT_COMMAND_BATCH);
        ...
        runOnUiThread(() -> {
            ...
            if (state == RemoteConnectionState.CONNECTED) {
                ActiveTerminalSessionStore.shared().connected(terminalSessionId,
                        selectedSessionTransport(), targetLabel(profile),
                        "", System.currentTimeMillis());
            } else if (state == RemoteConnectionState.DISCONNECTED) {
    ...
    public void onTextReceived(String text) {
        ...
        runOnUiThread(() -> {
            ...
            ActiveTerminalSessionStore.shared().append(
                    terminalSessionId, safe, System.currentTimeMillis());
    ...
    private void detectVendorIfNeeded() {
        if (vendorDetectionCompleted) return;
        ...
        DeviceProfile profile = new DeviceProfileStore(this).load();
        if (profile.getVendor() != fingerprint.getVendor()) {
            profile = profile.withVendor(fingerprint.getVendor());
            new DeviceProfileStore(this).save(profile);
            renderTargetBanner();
        }
        Toast.makeText(this, getString(R.string.vendor_detected,
                profile.getVendor().name(), fingerprint.getConfidence()), Toast.LENGTH_LONG).show();
    }
    ...
    private TerminalSessionTransport selectedSessionTransport() {
        return protocolSpinner != null && protocolSpinner.getSelectedItemPosition() == 1
                ? TerminalSessionTransport.TELNET : TerminalSessionTransport.SSH;
    }
