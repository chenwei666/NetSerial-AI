...
import com.chenwei666.netserial.automation.GuidedRunbookEngine;
import com.chenwei666.netserial.automation.GuidedRunbookPlan;
import com.chenwei666.netserial.automation.GuidedRunbookStep;
...
import com.chenwei666.netserial.diagnostics.IncidentCollectionEngine;
import com.chenwei666.netserial.diagnostics.IncidentCollectionPlan;
import com.chenwei666.netserial.diagnostics.IncidentCollectionStep;
...
import com.chenwei666.netserial.terminal.ActiveTerminalSnapshot;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

/** Reads redacted snapshots from existing terminals but never opens a connection or executes commands. */
public final class OperationsCenterActivity extends ThemedActivity {
    ...
    private String latestCommandBatch;
    ...
    @Override protected void onCreate(Bundle state) {
        ...
        sessionStatus = findViewById(R.id.operations_session_status);
        bindSpinners();
        input.setText(sanitize(getIntent().getStringExtra(EXTRA_CAPTURE)));
        findViewById(R.id.operations_read_session).setOnClickListener(view -> readCurrentSession());
        ...
        findViewById(R.id.operations_incident).setOnClickListener(view -> incidentPlan());
        findViewById(R.id.operations_ai_analyze).setOnClickListener(view -> analyzeWithAi());
    ...
    private void identify() {
        DeviceFingerprint result = identifyCapture(sanitize(input.getText().toString()));
    ...
    private DeviceFingerprint identifyCapture(String capture) {
        DeviceFingerprint result = new DeviceFingerprintEngine().identify(capture);
    ...
    private void readCurrentSession() {
        ActiveTerminalSnapshot snapshot = ActiveTerminalSessionStore.shared().latestConnected();
        if (snapshot == null) {
            sessionStatus.setText(R.string.operations_session_disconnected);
            output.setText(R.string.operations_session_required);
            return;
        }
        String capture = sanitize(snapshot.getText());
        if (capture.trim().isEmpty()) {
            refreshSessionStatus(snapshot);
            output.setText(R.string.operations_session_empty);
            return;
        }
        input.setText(capture);
        input.setSelection(input.length());
        DeviceFingerprint result = identifyCapture(capture);
        refreshSessionStatus(snapshot);
        output.setText(getString(R.string.operations_session_read_result,
                snapshot.getTargetLabel(), capture.length(), result.getVendor().name(),
                result.getConfidence(), result.getPlatform(), result.getEvidence()));
    }

    private void refreshSessionStatus() {
        refreshSessionStatus(ActiveTerminalSessionStore.shared().latestConnected());
    }

    private void refreshSessionStatus(ActiveTerminalSnapshot snapshot) {
        if (snapshot == null) {
            sessionStatus.setText(R.string.operations_session_disconnected);
            return;
        }
        sessionStatus.setText(getString(R.string.operations_session_connected,
                snapshot.getTransport().name(), snapshot.getTargetLabel(), snapshot.getText().length()));
    }
    ...
    private void generate() {
        try {
            ...
            latestCommandBatch = latestPlan.commandBatch();
            GuidedRunbookPlan guided = new GuidedRunbookEngine().prepare(latestPlan);
            output.setText(getString(R.string.operations_plan_result, latestPlan.getRisk().name(),
                    formatRunbook(guided), join(latestPlan.getStopConditions())));
        } catch (RuntimeException exception) {
            latestPlan = null;
            latestCommandBatch = null;
    ...
    private void healthPlan() {
        ...
        latestCommandBatch = plan.commandBatch();
        output.setText(getString(R.string.operations_health_result, plan.getRisk().name(), plan.commandBatch()));
    }

    private void incidentPlan() {
        IncidentCollectionPlan plan = new IncidentCollectionEngine().plan(selectedVendor());
        latestCommandBatch = plan.commandBatch();
        StringBuilder result = new StringBuilder(getString(R.string.operations_incident_result,
                plan.getRisk().name(), plan.getSteps().size()));
        String category = "";
        for (IncidentCollectionStep step : plan.getSteps()) {
            if (!category.equals(step.getCategory())) {
                category = step.getCategory();
                result.append("\n\n[").append(category).append(']');
            }
            result.append("\n").append(step.getCommand()).append("\n  ").append(step.getPurpose());
        }
        result.append("\n\n").append(getString(R.string.operations_stop_conditions))
                .append("\n").append(join(plan.getStopConditions()));
        output.setText(result.toString());
    }

    private void analyzeWithAi() {
        String diagnostic = sanitize(input.getText().toString());
        if (!output.getText().toString().trim().isEmpty()) {
            diagnostic += "\n\nLocal analysis or collection plan:\n" + sanitize(output.getText().toString());
        }
        if (diagnostic.trim().isEmpty()) {
            output.setText(R.string.operations_input_invalid);
            return;
        }
        startActivity(new Intent(this, AiCopilotActivity.class)
                .putExtra(AiCopilotActivity.EXTRA_DIAGNOSTIC_TEXT, diagnostic));
    }
    ...
    private void usePlan() {
        if (latestCommandBatch == null || latestCommandBatch.trim().isEmpty()) {
        ...
        setResult(Activity.RESULT_OK, new Intent().putExtra(RESULT_COMMAND_BATCH, latestCommandBatch));
        finish();
    }

    private static String formatRunbook(GuidedRunbookPlan plan) {
        StringBuilder result = new StringBuilder();
        for (GuidedRunbookStep step : plan.getSteps()) {
            if (result.length() > 0) result.append("\n\n");
            result.append('[').append(step.getSequence()).append("] ").append(step.getCommand())
                    .append("\nexpect=").append(step.getExpectedPromptPattern())
                    .append(" timeout=").append(step.getTimeoutSeconds()).append('s')
                    .append(" retry=").append(step.getMaxRetries())
                    .append(" failure=").append(step.getFailureAction());
        }
