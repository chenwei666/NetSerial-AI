...
import com.chenwei666.netserial.change.ChangeReadinessAssessment;
import com.chenwei666.netserial.change.ChangeReadinessValidator;
...
public final class ChangeTaskActivity extends ThemedActivity {
    ...
    private void startTask() {
        try {
            ...
            ChangeTask task = new ChangeTask(UUID.randomUUID().toString(), ticket.getText().toString(),
                    ...
                    now + minutes * 60_000L, ChangeTaskStatus.DRAFT, new ArrayList<>());
            ChangeReadinessAssessment readiness = new ChangeReadinessValidator().assessForStart(task);
            if (!readiness.isReady()) {
                status.setText(getString(R.string.change_readiness_failed,
                        join(readiness.getMissingRequirements())));
                return;
            }
            task = task.start(now);
            store.save(task);
    ...
    private void closeTask(boolean completed) {
        try {
            ...
            if (completed) {
                ChangeReadinessAssessment readiness = new ChangeReadinessValidator()
                        .assessForCompletion(task);
                if (!readiness.isReady()) {
                    status.setText(getString(R.string.change_closure_failed,
                            join(readiness.getMissingRequirements())));
                    return;
                }
            }
