...
- 开发人员：chenwei666
- 发布状态：本地开发候选；尚未推送、合并或发布，GitHub Latest 仍为 V0.9.0。

### 新增功能

- 运维中心新增“当前连接”状态卡和“一键读取当前设备并识别”入口。
- USB、SSH、Telnet 终端新增统一的进程内活动会话快照，运维中心可读取最近活动且仍在线的会话。
- 一键读取后自动清理 ANSI、再次脱敏、填入分析文本，并复用既有 H3C、Huawei、Cisco、Ruijie 指纹引擎切换厂商页面与设备档案。
- 增加中英文连接状态、空会话、未连接、读取成功和操作说明。

### 问题修复与优化

- 修复从全局工具箱进入运维中心时只能手工粘贴输出、无法复用当前终端内容的问题。
- 多会话同时存在时按最近活动顺序选择，断开的会话不会被读取；清屏同步清除共享快照。
- 会话桥接层限定最多 4 个会话、每会话最近 100,000 字符，不持久化、不持有连接对象，也不暴露命令执行回调。

### 删除内容

- 无；V0.9.0 与更早版本功能和归档均保留。

### 修改文件与影响模块

- `terminal/ActiveTerminalSessionStore.java`、`ActiveTerminalSnapshot.java`、`TerminalSessionTransport.java`：只读会话桥接领域层。
- `TerminalFragment.java`：USB 会话注册、接收、发送脱敏副本、清屏和断开同步。
- `RemoteTerminalActivity.java`：SSH/Telnet 会话注册、接收、状态、清屏和断开同步。
- `OperationsCenterActivity.java`、`activity_operations_center.xml`、中英文字符串：一键读取、状态显示和自动识别。
- `ActiveTerminalSessionStoreTest.java`：最近活动、断开排除、有界缓冲、ANSI 清理和敏感信息脱敏测试。
- `app/build.gradle`、`scripts/build.ps1`、README 与 docs：V0.9.1 版本、隔离构建和交付文档。

### 数据库、接口、配置与兼容性

- 数据库、外部 API 和必填配置：无变更。
- 进程内接口：新增只读活动会话快照；应用进程退出后自动消失，不写入 SharedPreferences、数据库或文件。
- 版本配置：versionCode 11、versionName 0.9.1；保持 minSdk 21、targetSdk/compileSdk 36。
- 使用既有生产证书，可从 V0.3.1–V0.9.0 覆盖升级并保留应用私有数据。
...
- 升级方式：安装同签名 APK 后连接设备，在终端产生只读输出，再进入运维中心点击一键读取；正式发布前仍需用户授权推送、合并和 Release。

## V0.9.0 - 2026-08-25

- 开发人员：chenwei666
- 发布状态：PR #9 已合并到 `main`，V0.9.0 已发布为 GitHub Latest 正式版；自动化生产门禁已通过，真实设备/交换机/AI 账号现场验收尚待确认。

### 新增功能

- 新增完整应用内 AI 多轮对话：新建、选择、重命名、删除、停止、重试、复制、脱敏分享、终端上下文和设备记忆。
- 新增 `AiChatProvider` 抽象及 OpenAI-compatible、Anthropic Messages、HTTPS Ollama 对话适配器；全部复用既有档案、模型、凭据与 HTTPS Transport。
- 新增活动 AI 档案优先、每厂商最多两次瞬时错误重试、跨厂商故障转移和取消不转移策略。
- 新增有界对话领域模型与 JSON Codec；Android 6.0+ 使用 Android Keystore/AES-GCM 加密历史，Android 5.x 仅使用内存。
- 新增显式 Markdown 围栏命令提取、网络 CLI 前缀白名单和确定性 R0–R4 重新分级；R4 在 AI 页面保持阻断。
- 新增一键故障取证计划，覆盖身份、时钟、系统健康、接口、二层、三层、邻居和日志证据。
- 新增结构化只读运行手册合同：提示符正则、20 秒超时、一次重试、首错即停和停止条件。
- 新增配置漂移评估，输出 NONE/LOW/MEDIUM/HIGH、变更行数和敏感变更数。
- 安全/合规检查新增 SSHv1、弱 SSH 算法、仅旧版 SNMP、AAA 缺失和集中日志缺失规则。
- 新增变更就绪/闭环验证：启动前要求预检查、命令、验证、回退计划；完成前要求执行与验证证据。

### 问题修复

- 修复原 AI 页面只能生成单轮结构化命令计划、无法持续追问或保留受控上下文的问题。
- 修复 Android 5.x 因平台密钥库不可用而阻断无凭据 HTTPS Ollama 对话的问题；本地模型可继续使用，云端密钥操作保持关闭且绝不降级为明文。
- 修复 AI 厂商适配层只能解析命令 JSON、不能安全解析普通对话文本和内容分片的问题。
- 修复配置差异只显示增删行、无法突出管理面与转发面敏感漂移的问题。
- 修复变更任务可在缺少预检查/验证计划或缺少执行证据时关闭的问题。

### 优化内容

- AI 界面改为设备/厂商摘要、消息气泡、状态、上下文开关和固定输入区，低频操作收入工具栏菜单。
- 对话请求最多保留最近 24 条消息，并跳过开头孤立助手消息、合并连续同角色消息，提高多厂商兼容性。
- 回复和分享再次脱敏；API Key 仍只通过短生命周期字符数组进入请求，不进入对话模型、缓存或文档。
- 运维中心复用已有健康、剧本、合规、配置和终端门禁模块，未增加重复连接层或隐藏执行通道。

### 删除内容

- 无。V0.8.0 全部功能、接口和历史版本目录均保留。

### 修改文件与影响模块

- `app/build.gradle`：versionCode 10、versionName 0.9.0。
- `ai`：对话领域模型、Codec、厂商适配、故障转移、加密历史和命令提取。
- `diagnostics`：一键故障取证计划。
- `automation`：结构化运行手册合同。
- `config`：配置漂移评估与 Diff 报告集成。
- `compliance`：安全基线规则扩展。
- `change`：启动/完成就绪验证。
- UI/resources：AI 对话页、运维中心、配置对比和中英文字符串。
- docs/scripts：V0.9.0 构建目录、README、架构、安全、兼容性、测试、发布和交接文档。

### 数据库、接口与配置变更

- 数据库变更：无。
- 持久化变更：新增 `netserial-ai-chat-history-v1` 加密记录；最多 12 会话、每会话 50 消息、总序列化 400,000 字符。
- 外部接口：新增同一用户配置端点上的多轮文本 POST；模型目录 GET、ProviderProfile JSON 和凭据别名格式不变。
- 配置变更：无新增必填配置；原有 AI 档案、密钥、模型缓存、设备档案、快照和变更任务可继续使用。

### 安全与兼容性

- AI 请求前执行 ANSI 清理、敏感字段脱敏、提示注入隔离、消息数和字符数限制；响应执行脱敏与大小限制。
- AI 不自动执行命令；R4 不可装载，R1–R3 仍须人工确认并继续经过终端目标与维护窗口门禁。
- Android 5.0/API 21 至 Android 16/API 36 目标兼容性保持；加密历史需要 Android 6.0/API 23。
- 使用与 V0.3.1–V0.8.0 相同生产证书时可覆盖升级并保留应用私有数据。

### 升级方式

1. 仅从匹配的 GitHub Release 下载 APK 并核对 SHA-256。
2. 确认签名证书指纹与既有生产升级链一致。
3. 覆盖安装后检查 AI 档案、模型、设备档案、变更任务和终端设置。
4. 首次打开 AI 对话时确认是否启用加密历史与设备记忆。

### 测试结果、已知问题和备注

- 59 个测试类、181 项测试全部通过，0 失败、0 错误、0 跳过；Debug/Release Lint、Debug/Release APK、ZIP 对齐和 V1/V2/V3 签名通过。
- Release Lint 为 0 Error、138 Warning；警告属于既有资源/样式/Autofill/依赖基线，新增 AI 对话源码与布局没有 Lint 警告。
- APK SHA-256：`b4c4040cfc2f1895404dd0c9b6f8024f3824f4a1baf161621e132e504ee29697`；证书 SHA-256 与既有生产链一致。
- 真实 Android UI、USB OTG、四厂商交换机、跳板机、付费 AI 账号和不同厂商响应差异仍需授权现场验收。
- NETCONF/RESTCONF、gNMI/YANG-Push、Syslog/SNMP Trap、主动多设备拓扑和实时抓包不属于 V0.9.0 已实现范围。

## V0.8.0 - 2026-08-25

- 开发人员：chenwei666
- 发布状态：开发完成候选；Debug/Release 自动化门禁已通过，正式发布前仍需现场验收。

### 新增功能

- AI 厂商从 8 类扩展到 18 类，新增智谱 GLM、豆包、腾讯 TokenHub/混元、百度千帆、MiniMax、硅基流动、Groq、Mistral、xAI 和 OpenRouter。
- 新增上游模型目录同步、模型搜索下拉、显式刷新与取消、本地非秘密缓存、缓存清理及手工模型名回退。
- 新增单目标最多 16 个 TCP 端口的列表/范围检查、常用端口填充、地址属性摘要、结果复制和系统分享。
- 新增统一 Material 3 功能任务卡片，覆盖主页、工具/设置中心和全局功能搜索。

### 问题修复

- 修复 AI 厂商不完整且模型只能手工记忆输入的问题。
- 修复网络工具所有输入、操作与结果堆叠在单个长表单中，关键动作不直观的问题。
- 修复多个网络任务可连续排队导致旧结果覆盖新操作的问题。
- 修复继承构建脚本复用 V0.6.0 临时目录造成 Gradle 锁冲突的问题。
- 修复模型缓存键使用普通整数哈希可能发生碰撞的问题，改用完整 SHA-256 作用域键。
- 修复 TFTP 临时服务关闭时活动传输套接字与工作线程可能尚未释放的问题。
- 修复模型刷新使用既有凭据时可能把密钥发送到尚未保存的新厂商或新端点的问题。
- 修复 Qwen 模型目录端点与 JSON 格式不同于聊天兼容端点、导致上游列表刷新失败的问题。
- 修复切换 AI 档案后旧模型请求仍可能覆盖新档案下拉列表的问题。
- 修复手工编辑 Endpoint 时未使旧模型请求和连接测试失效，以及连接测试可能复用旧凭据访问新目的地的问题。
- 修复未保存新档案共用预览缓存别名、可能看到另一未保存账号模型的问题；未保存档案现在只使用会话内刷新结果。
- 修复 DNS 失败被批量端口检查误报为全部关闭，以及页面销毁后连接循环未及时停止的问题。
- 修复取消旧 TCP 探测与新探测交错时，旧任务可能清除新活动 Socket 引用的问题。

### 优化内容

- AI 设置按档案、厂商模型、安全凭据分组；网络工具按快速诊断、结果、CIDR 和离线工具分组。
- 模型目录响应去重并按名称排序，限制最多 1000 个、单个名称最多 256 字符。
- TCP 多端口检查限制单端口 1.5 秒，避免长时间阻塞。
- ProviderProfile 持久化结构保持不变，旧版配置可直接继续使用。
- TFTP 下载计数改为线程可见，并增加连续 10 次真实 UDP 传输/关闭回归测试。
- 批量端口和地址摘要返回结构化结果，由 Android 中英文资源负责呈现；TFTP 页面停止操作改为非阻塞请求。

### 删除内容

- 无。V0.7.0 既有功能与历史版本目录全部保留。

### 修改文件与影响模块

- app/build.gradle：versionCode 9、versionName 0.8.0。
- ai：厂商目录、预置端点、模型目录 Transport/Resolver/Codec/Service/Cache。
- network：端口列表解析、受控 TCP 检查与地址摘要。
- UI：AI 设置、网络工具、主页、功能中心和搜索卡片。
- resources/docs：中英文资源、README、架构、安全、测试、发布和版本记录。

### 数据库、接口与配置变更

- 数据库变更：无。
- 持久化变更：新增 ai_model_catalog_v1 SharedPreferences，只保存模型标识与获取时间。
- 外部接口：新增用户主动发起的 HTTPS GET 模型目录请求；既有聊天接口不变。
- 配置变更：新增厂商预置；既有 ProviderProfile JSON 无迁移。

### 安全与兼容性

- API Key 不写入模型缓存、日志或文档；认证头模式隔离并拒绝控制字符。
- 模型同步只允许 HTTPS 且禁止重定向，受超时和响应大小限制。
- 端口检查仅面向一个显式目标，最多 16 个端口，不支持网段扫描。
- Android 5.0/API 21 至 Android 16/API 36 目标兼容性保持不变。

### 升级方式、已知问题与备注

- 使用与 V0.3.1 至 V0.7.0 相同生产证书签名后，可覆盖安装并保留应用私有数据。
- 部分厂商账户、区域或兼容网关可能不开放模型目录；此时使用缓存或手工模型名。
- 真实手机、USB、交换机、系统网络工具和付费 AI 账号仍需授权现场验收。
- 自动化验收：50 个测试类、163 项测试全部通过；Debug/Release Lint 和构建、ZIP 对齐、V1/V2/V3 生产签名验证通过。
