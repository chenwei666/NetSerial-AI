...
- Android 后续覆盖升级必须使用与 V0.3.1/V0.4.0/V0.5.0/V0.6.0/V0.7.0/V0.8.0 相同的生产证书。
...
Every future in-place Android update must use the same V0.3.1/V0.4.0/V0.5.0/V0.6.0/V0.7.0/V0.8.0 production certificate. Keep the `.p12` keystore and its passphrase in separate offline backups. The DPAPI ciphertext is bound to the current Windows user and is not a portable backup. Initialize signing once, build with `scripts\build.ps1 -Release`, verify V1/V2/V3 signatures, ZIP alignment, package metadata, and SHA-256, then download the published asset and verify it again.
