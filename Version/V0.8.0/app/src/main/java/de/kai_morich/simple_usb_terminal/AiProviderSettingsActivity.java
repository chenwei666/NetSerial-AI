...
import com.chenwei666.netserial.ai.AiModelCacheStore;
import com.chenwei666.netserial.ai.AiModelCatalogService;
...
public final class AiProviderSettingsActivity extends ThemedActivity {
    ...
    private final AiModelCatalogService modelCatalogService = new AiModelCatalogService();
    ...
    private AiModelCacheStore modelCacheStore;
    ...
    private TextView modelCatalogStatus;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        modelCacheStore = new AiModelCacheStore(this);
    ...
    private void bindViews() {
        ...
        refreshModelsButton = findViewById(R.id.ai_refresh_models_button);
        testProgress = findViewById(R.id.ai_test_progress);
        modelProgress = findViewById(R.id.ai_model_progress);
        modelCatalogStatus = findViewById(R.id.ai_model_catalog_status);
    }
    ...
    private void bindActions() {
        ...
        refreshModelsButton.setOnClickListener(view -> {
            if (modelCancellation == null) refreshModels();
            else modelCancellation.cancel();
        });
        findViewById(R.id.ai_clear_model_cache_button).setOnClickListener(view -> {
            try {
                modelCacheStore.clearAll();
                modelInput.setAdapter(null);
                modelCatalogStatus.setText(R.string.ai_model_cache_cleared);
            } catch (RuntimeException exception) {
                showStatus(R.string.ai_model_cache_clear_failed);
            }
        });
        modelInput.setOnClickListener(view -> {
            if (modelInput.getAdapter() != null) modelInput.showDropDown();
        });
    ...
    private ProviderProfile buildProfileSafely(String alias) {
        try {
            return buildProfile(alias);
        } catch (RuntimeException ignored) {
            return null;
        }
    }

    private void renderCachedModels(ProviderProfile profile) {
        if (profile == null || modelCacheStore == null) return;
        CachedAiModels cached = modelCacheStore.load(profile);
        if (cached.isEmpty()) {
            modelCatalogStatus.setText(R.string.ai_models_not_loaded);
            return;
        }
        applyModelSuggestions(cached.getModels());
        modelCatalogStatus.setText(getString(
                R.string.ai_models_cached, cached.getModels().size()));
    }
    ...
    private void refreshModels() {
        ...
        modelWorker.execute(() -> {
            try {
                List<String> models;
                if (!noKeyRequired && typedCredential.length == 0) {
                    models = credentialService.withCredential(existing,
                            credential -> modelCatalogService.fetch(profile, credential, cancellation));
                } else {
    ...
    private boolean hasCredentialSafely(ProviderProfile profile) {
        try {
    ...
    private void finishModelSync(ProviderProfile profile, List<String> models,
        ...
        if (failure == null && models != null) {
            ...
            modelCatalogStatus.setText(getString(R.string.ai_models_loaded, models.size()));
        ...
        if (failure instanceof com.chenwei666.netserial.ai.AiProviderException) {
            ...
            showStatus(R.string.ai_models_load_failed);
        }
    }

    private void applyModelSuggestions(List<String> models) {
        ArrayAdapter<String> adapter = new ArrayAdapter<>(this,
                android.R.layout.simple_dropdown_item_1line, models);
        modelInput.setThreshold(0);
        modelInput.setAdapter(adapter);
    }

    private void setModelSyncRunning(boolean running) {
        modelProgress.setVisibility(running ? View.VISIBLE : View.GONE);
        refreshModelsButton.setText(running ? R.string.ai_cancel_model_sync
                : R.string.ai_refresh_models);
        modelCatalogStatus.setText(running ? R.string.ai_models_loading
