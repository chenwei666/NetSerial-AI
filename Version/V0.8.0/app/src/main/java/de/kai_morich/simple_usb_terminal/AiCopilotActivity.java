...
import com.chenwei666.netserial.ai.AiProviderCandidate;
import com.chenwei666.netserial.ai.AiProviderFactory;
import com.chenwei666.netserial.ai.AiRequest;
import com.chenwei666.netserial.ai.AiFailoverResult;
...
import com.chenwei666.netserial.ai.FailoverAiCopilot;
...
public final class AiCopilotActivity extends ThemedActivity {
    ...
    @Override protected void onCreate(Bundle state) {
        ...
        String diagnostic = getIntent().getStringExtra(EXTRA_DIAGNOSTIC_TEXT);
        if (diagnostic != null && !diagnostic.trim().isEmpty()) {
            String safeDiagnostic = new SensitiveTextRedactor().redact(
                    new AnsiTextSanitizer().sanitize(diagnostic));
            if (safeDiagnostic.length() > 8_000) {
                safeDiagnostic = safeDiagnostic.substring(safeDiagnostic.length() - 8_000);
            }
            intentInput.setText(getString(R.string.ai_diagnostic_prompt, safeDiagnostic));
    ...
    private void generateProposal() {
        ...
        try {
            ...
            profiles = orderedProfiles(state);
            if (profiles.isEmpty()) {
        ...
        executor.submit(() -> {
            try {
                ...
                List<AiProviderCandidate> candidates = new ArrayList<>();
                for (ProviderProfile profile : profiles) {
                    AiProvider provider = AiProviderFactory.create(profile, credentials);
                    candidates.add(new AiProviderCandidate(profile.getProviderId() + " / " + profile.getModel(),
                            new SafeAiCopilot(provider, RuleBasedExecutionGuard.createDefault())));
                }
                AiFailoverResult failover = new FailoverAiCopilot(candidates, 2).propose(
                        new AiRequest(intent, device.getVendor(), device.getCliMode(), requestContext));
                CommandPlan plan = failover.getPlan();
                ...
                runOnUiThread(() -> renderPlan(plan, finalMemoryContext,
                        finalMemorySaved, finalMemoryRejected, failover.getProviderAlias(),
                        failover.getAttempts().size()));
            } catch (RuntimeException exception) {
                runOnUiThread(() -> {
                    setBusy(false);
    ...
    private void renderPlan(CommandPlan plan, String recalledMemory,
                            ...
                            String providerAlias, int attemptCount) {
        if (isFinishing() || isDestroyed()) return;
        setBusy(false);
        StringBuilder summary = new StringBuilder(getString(
                R.string.ai_copilot_result_count, plan.getSteps().size()));
        summary.append('\n').append(getString(R.string.ai_failover_summary, providerAlias, attemptCount));
