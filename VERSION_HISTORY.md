...
- 从 V0.3.1 开始，V0.3.1 至 V0.7.0 使用同一生产证书；V0.8.0 正式发布时也必须使用该证书，才能覆盖升级并保留应用私有数据。
- 安装前必须从对应 Release 下载附件并核对 SHA-256；不要安装来源不明或哈希不一致的 APK。
- V0.7.0 是当前 GitHub `Latest` 正式版。V0.8.0 尚未发布；部署到生产网络前仍建议在授权手机、USB 串口芯片和目标交换机上完成现场验收。
